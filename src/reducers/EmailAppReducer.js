...
export default (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case FETCH_EMAILS:
      const allEmails =
        state.allEmail && state.allEmail.filter((email) => !email.deleted);
    ...
    case SEARCH_EMAIL:
      if (action.payload === "") {
        return {
          ...state,
          emails: state.allEmail.filter((email) => !email.deleted),
        };
      } else {
    ...
    case GET_SENT_EMAILS:
      const sentEmails =
        state.allEmail &&
        state.allEmail.filter((email) => !email.deleted && email.folder === 1);
    ...
    case GET_INBOX:
      const inbox =
        state.allEmail &&
        state.allEmail.filter((email) => !email.deleted && email.folder === 0);
    ...
    case GET_DRAFTS_EMAILS:
      const drafts =
        state.allEmail &&
        state.allEmail.filter((email) => !email.deleted && email.folder === 2);
    ...
    case GET_SPAM_EMAILS:
      const spamEmails =
        state.allEmail &&
        state.allEmail.filter((email) => !email.deleted && email.folder === 3);
      return { ...state, emails: spamEmails, selectedFolder: 3 };

    // trash emails
    case GET_TRASH_EMAILS:
      const trashEmails =
        state.allEmail && state.allEmail.filter((email) => email.folder === 4);
      ...
      const labelsEmails = state.emails.map((email) => {
        if (email.selected) {
          if (email.email_labels.includes(action.payload.value)) {
            email.email_labels.splice(
              email.email_labels.indexOf(action.payload.value),
              1
            );
            ...
            return {
              ...email,
              email_labels: email.email_labels.concat(action.payload.value),
            };
          }
        } else {
          return email;
